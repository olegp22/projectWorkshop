...
def create_submission_p2p(
    ...
    # Получаем всех студентов кроме автора
    ...
    # Берем count_of_inspectors самых свободных студентов
    reviewer_counts = (
        db.query(
            GroupMember.user_id,
            func.count(SubmissionReviewer.id).label("total")
...
def submit_review(db: Session, submission_id: int, reviewer_id: int, review_data: ReviewCreate):
    ...
    for g in review_data.grades:
        ...
        criterion = db.query(Criterion).filter(
            Criterion.id == g.criterion_id
        ).first()

        if not criterion:
            raise HTTPException(
                status_code=400,
                detail=f"Критерий {g.criterion_id} не существует"
            )
...
def get_submission_details(db: Session, submission_id: int):
    ...
    # Получаем все проверки этой работы
